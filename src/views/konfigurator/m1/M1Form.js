...
import TableDataGridM1 from './TableDataGridM1'

// ** Third Party Imports
import calculateM1 from 'src/pages/konfigurator/m1/berechnung/calc'
import AddKonfigDrawer from 'src/pages/components/konfigurator/AddKonfigDrawer'

// ** Icons Imports
import { FormGroup } from '@mui/material'

import M1DropDownGehaeuse from './M1DropDownGehaeuse'

import M1DropDownLED from './M1DropDownLED'

import M1DropDownTreiber from './M1DropDownTreiber'
...
const SpotForm = props => {
  ...
  useEffect(() => {
    fetch('/api/m1/getCalc', { method: 'POST', body: JSON.stringify(dropdownCtx.dropDownValues) })
      .then(response => {
          ...
          return {
            ...
            calc: JSON.parse(
              calculateM1({
                ...
                E_Lampenplatz: parseInt(data[0].anzahl_leuchtmittel),
                E_Min_Anzahl: parseInt(data[0].min_anzahl),
                E_Max_Anzahl: parseInt(data[0].max_anzahl),
      ...
      .then(calc => {
        ...
        fetch('/api/m1/getArtikelGehaeuse', { method: 'POST', body: JSON.stringify(calc.calc) })
          .then(response => response.json())
          .then(data => {
            setgeheause(data[0])
          })

        fetch('/api/m1/getArtikelLED', { method: 'POST', body: JSON.stringify(calc.calc) })
          .then(response => response.json())
          .then(data => {
            setled(data[0])
          })

        fetch('/api/m1/getArtikelTreiber', { method: 'POST', body: JSON.stringify(calc.calc) })
          .then(response => response.json())
          .then(data => {
            settreiber(data[0])
          })
  ...
  return (
    <Fragment>
      <AddKonfigDrawer open={addKonfigOpen} toggle={toggleAddKonfigDrawer} datagrid={<TableDataGridM1 />} />
      <Card>
        ...
        <form onSubmit={e => e.preventDefault()}>
          <Grid container spacing={2} alignItems={'flex-start'} justifyContent={'start'} sx={{ p: 5 }}>
            ...
            <M1DropDownGehaeuse></M1DropDownGehaeuse>
            <Grid xs={12} lg={12} sx={{ borderTop: 2, borderColor: '#000' }}>
              <Table sx={{ maxWidth: '2250px' }}>
                <TableBody>
                  <TableRow>
            ...
            <M1DropDownTreiber></M1DropDownTreiber>
